...
fn images_to_dds_sequential(files: Vec<String>, source_dir: String, output_path: String, dds_format: image_dds::ImageFormat) -> anyhow::Result<()> {
...
fn images_to_dds_parallel(files: Vec<String>, source_dir: String, output_path: String, dds_format: image_dds::ImageFormat) -> anyhow::Result<()> {
    // to prevent processing dds->dds, filter out dds files from files
    let files: Vec<String> = files.into_iter().filter(|path| !path.ends_with(".dds")).collect();

    // Use par_chunks to process files in parallel batches
    files.par_chunks(5).try_for_each(|chunk| -> anyhow::Result<()> {
        chunk.iter().try_for_each(|path_string| -> anyhow::Result<()> {
            let cloned_path = path_string.clone();
            let mut source_relative_path = diff_paths(Path::new(&cloned_path), Path::new(&source_dir)).ok_or_else(||
...
fn dds_to_images_sequential(files: Vec<String>, source_dir: String, output_path: String, output_format: ImageFormatEnum) -> anyhow::Result<()> {
    // to prevent processing dds->dds, filter out dds files from files
...
fn dds_to_images_parallel(files: Vec<String>, source_dir: String, output_path: String, output_format: ImageFormatEnum) -> anyhow::Result<()> {
    // to prevent processing dds->dds, filter out dds files from files
    let sss: &str = output_format.into();
    let files: Vec<String> = files.iter().filter(|path| !path.ends_with(sss)).cloned().collect();
...
mod tests {
    ...
    fn test_images_to_dds() {
        ...
        let convert_result = images_to_dds_sequential(
            files,
            source_dir,
            output_path,
            dds_format,
    ...
    fn test_dds_to_images() {
        ...
        let convert_result = dds_to_images_sequential(
            files,
            source_dir,
            output_path,
            output_format,
    ...
    fn sex() {
        ...
        assert!(convert_result.is_ok());
        println!("Time elapsed in images_to_dds_sequential() is: {:?}", duration);

        let start = Instant::now();
        let convert_result = images_to_dds_parallel(
            files_clone,
            source_dir_clone,
            output_path_clone,
            dds_format,
        ...
        assert!(convert_result.is_ok());
        println!("Time elapsed in images_to_dds_parallel() is: {:?}", duration);
    }
}